...
ControlCommand.solve = (context, message, argument) => {
    ...
    if (["on", "off", "clear"].includes(action) && context.privilegeLevel < 50 /* moderator */) {
...
function toggleGroupActive(isActive: boolean, groupId: string) {
    const storage = JSON.parse(Extension.storageGet(`${STORAGE_NAMESPACE}_${groupId}`) || "{}");
    storage["active"] = isActive;
    Extension.storageSet(`${STORAGE_NAMESPACE}_${groupId}`, JSON.stringify(storage));
}
...
function getRandomMessage(groupId: string): { speaker: string, content: string } | null {
    const storage = JSON.parse(Extension.storageGet(`${STORAGE_NAMESPACE}_${groupId}`) || "{}");
