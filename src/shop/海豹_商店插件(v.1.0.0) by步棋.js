...
class Rucksack {
    ...
    constructor(ctx) {
        ...
        this.money = itemAll[this.userId] ? itemAll[this.userId]["money"] : 50;
    }
    save() {
        let itemAll = JSON.parse(ext.storageGet("backpacks") || "{}");
...
cmdShow.solve = (ctx, msg, args) => {
    ...
    if (name !== "背包" && name !== "商店") {
        ...
        seal.replyToSender(ctx, msg, `${msg.sender.nickname}的背包（余额 ${backpack.money}）:\n${items}`);
    }
    else {
        let shop = new Shop();
